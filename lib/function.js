...
const GIFBufferToVideoBuffer = async (image) => {
    ...
    await fs.writeFileSync(`./EMMYHENZ/trash/${filename}.gif`, image)
    child_process.exec(
        `ffmpeg -i ./EMMYHENZ/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./EMMYHENZ/trash/${filename}.mp4`
    )
    await sleepy(4000)

    var buffer5 = await fs.readFileSync(`./EMMYHENZ/trash/${filename}.mp4`)
    Promise.all([unlink(`./EMMYHENZ/trash/${filename}.mp4`), unlink(`./EMMYHENZ/trash/${filename}.gif`)])
    return buffer5
}
